...
function consultarEstatus(codigo) {
  ...
  // Limpiamos el código recibido para evitar errores de espacios o tipos
  const codigoBuscado = String(codigo).trim();

  for (let i = 1; i < datos.length; i++) {
    // Convertimos el valor de la columna 7 (Código UDG) a texto para comparar
    const codigoEnCelda = String(datos[i][7]).trim();
    
    if (codigoEnCelda === codigoBuscado) {
      resultados.push({ 
        folio: datos[i][0],      // <--- IMPORTANTE: Ahora sí enviamos el Folio
        taller: datos[i][3], 
        asistencias: datos[i][16], 
        porcentaje: datos[i][17] 
      });
    }
  }
  return resultados;
