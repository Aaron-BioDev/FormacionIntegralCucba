...
/* =======================================
 * BLINDAJE 1: CAPTCHA E INICIALIZACIÓN
 * ======================================= */
let correctAnswer = 0;
...
if (form) {
    form.addEventListener("submit", async (e) => {
        ...
        /* =======================================
         * BLINDAJE 2: VALIDACIONES FRONT-END
         * ======================================= */
        // A. Verificar Honeypot Invisible
        const trap = document.getElementById("trap_bot");
        if (trap && trap.value !== "") {
            alert("Actividad de BOT detectada. Envío bloqueado de forma permanente.");
            return;
        }

        // B. Verificar Captcha Matemático
        const userAnswer = parseInt(document.getElementById("mathCaptcha").value);
        if (userAnswer !== correctAnswer) {
            alert("🛡️ La suma de seguridad es incorrecta. Por favor, inténtalo de nuevo.");
            initCaptcha();
            document.getElementById("mathCaptcha").value = "";
            return;
        }

        // C. Rate Limiting (Anti-Spam de 30 segundos)
        const now = Date.now();
        const lastSubmit = localStorage.getItem("lastSubmitTime");
        if (lastSubmit && now - lastSubmit < 30000) {
            alert("⏳ Estás enviando registros demasiado rápido. Por favor, espera 30 segundos antes de intentar de nuevo.");
            return;
        }

        // D. Verificación de Cupo In-RAM (Anti-Bypass Element)
        ...
        if (tallerInfo && !tallerInfo.esIlimitado && tallerInfo.disponibles <= 0) {
            alert("🚨 Error: Forzamiento detectado. Este taller ya no tiene lugares disponibles.");
            return; // Bloquea todo
        }
