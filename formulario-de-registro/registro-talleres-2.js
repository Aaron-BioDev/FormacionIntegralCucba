...
function generarPDF() {
    ...
    estiloPrint.innerHTML = `@media print { body * { visibility: hidden; } #ticket-exito, #ticket-exito * { visibility: visible; } #ticket-exito { position: absolute; left: 0; top: 0; width: 100%; border: none !important; box-shadow: none !important; } }`;
...
document.getElementById('formulario').addEventListener('submit', function(e) {
    ...
    fetch(urlDestino, { method: 'POST', mode: 'no-cors', body: datosEnvio })
    .then(() => {
        ...
        resp.innerHTML = `<div id="ticket-exito" style="background: white; border: 4px solid #1a73e8; border-radius: 20px; padding: 25px; margin-top: 20px; text-align: center; box-shadow: 0 10px 25px rgba(0,0,0,0.15); font-family: Arial, sans-serif;"><h2 style="color: #1a73e8; margin-top: 0;">✅ ¡REGISTRO EXITOSO!</h2><p style="color: #555; margin-bottom: 20px;">Presenta este QR oficial en cada sesión</p><div style="background: #f8f9fa; padding: 15px; border-radius: 15px; display: inline-block; border: 1px solid #ddd; margin-bottom: 15px;"><img src="${qrPrincipal}" alt="QR Asistencia" style="width: 250px; height: 250px; display: block;" onerror="this.onerror=null; this.src='${qrRespaldo}';"></div><div style="background: #1a73e8; color: white; padding: 12px; border-radius: 10px; font-family: monospace; font-size: 1.3em; font-weight: bold;">${miFolio}</div><div style="text-align: left; margin-top: 20px; padding-top: 15px; border-top: 2px dashed #eee; font-size: 0.9em;"><p style="margin: 4px 0;"><b>📍 Taller:</b> ${nombreTallerTexto}</p><p style="margin: 4px 0;"><b>🏢 Coordinación:</b> ${coord}</p></div><div style="background: #fff3cd; color: #856404; padding: 10px; border-radius: 8px; margin-top: 20px; font-weight: bold; font-size: 12px; border: 1px solid #ffeeba;">📸 TOMA CAPTURA DE PANTALLA</div><button onclick="generarPDF()" style="width: 100%; background: #34a853; color: white; border: none; padding: 12px; border-radius: 10px; margin-top: 10px; font-weight: bold; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px;">📄 Guardar como PDF / Imprimir</button></div>`;
    ...
    .finally(() => { btn.disabled = false; btn.innerHTML = "Confirmar Registro ✉️"; });
});